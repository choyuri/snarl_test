...
confirm() ->
    ...
    {ok, UUID1} = rt_snarl:user_add(Node1, ?USER1),

    ?assertEqual({ok, user(?USER1 ,UUID1)},
                 rt_snarl:user_get(Node1, UUID1)),
    ...
    wait_and_validate(Nodes, [Node2, Node3, Node4], UUID1),

    lager:info("taking Node 2 down"),
    rt:stop(Node2),
    ?assertEqual(ok, rt:wait_until_unpingable(Node2)),
    wait_and_validate(Nodes, [Node3, Node4], UUID1),

    lager:info("bringing Node 1 up"),
    rt:start(Node1),
    ok = rt:wait_until_pingable(Node1),
    wait_and_validate(Nodes, [Node1, Node3, Node4], UUID1),
    ...
    rt:leave(Node1),
    ?assertEqual(ok, rt:wait_until_unpingable(Node1)),
    wait_and_validate([Node2, Node3, Node4], UUID1),

    lager:info("leaving Node 2"),
    rt:leave(Node2),
    ?assertEqual(ok, rt:wait_until_unpingable(Node2)),
    wait_and_validate([Node3, Node4], UUID1),
...
wait_and_validate(RingNodes, UpNodes, UUID1) ->
    ...
    [?assertEqual({ok, user(?USER1 ,UUID1)},
                  rt_snarl:user_get(Node, UUID1)) || Node <- UpNodes],
    done.
