...
confirm() ->
    ...
    {ok, {Ps1, Gs1, Ks, Os}} = rpc:call(Node, snarl_user, gcable, [UUID]),
    ...
    {ok, {Ps2, Gs2, Ks, Os}} = rpc:call(Node, snarl_user, gcable, [UUID]),
    ...
    {ok, {Ps3, Gs3, Ks, Os}} = rpc:call(Node, snarl_user, gcable, [UUID]),
    ?assertEqual(1, length(Ps3)),
    ?assertEqual(1, length(Gs3)),
    {ok, GCed} = rpc:call(Node, snarl_user, gc, [UUID, {Ps3, Gs3, Ks, Os}]),
    lager:debug("GC freed ~p bytes.", [GCed]),
    {ok, {Ps4, Gs4, Ks, Os}} = rpc:call(Node, snarl_user, gcable, [UUID]),
